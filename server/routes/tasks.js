...
router.get('/', async (req, res, next) => {
  try {
    ...
    // Filter in JS to avoid nested sql`` parameter issues
...
router.post('/', authorize('admin', 'manager', 'lead'), [
  ...
  try {
    const {
      ...
      priority, dueDate, timeEstimate, tags, assigneeIds
    } = req.body;

    const [task] = await sql`
      INSERT INTO tasks (
        title, description, project_id, status,
        priority, due_date, time_estimate, tags, created_by
      ) VALUES (
        ${title},
        ${description || null},
        ${projectId},
      ...
      for (const uid of assigneeIds) {
        ...
        if (uid !== req.user.id) {
          await sql`
            ...
            VALUES (${uid}, 'assign', 'New task assigned', ${`You were assigned to "${title}"`})
          `;
        }
      }
    }
    ...
    await sql`
      ...
      VALUES (
        ${req.user.id}, ${projectId}, ${task.id},
        'task_created', ${JSON.stringify({ title })}
      )
    `;
  ...
  try {
    ...
    if (req.user.role === 'member') {
      const [assigned] = await sql`
        ...
        WHERE task_id = ${req.params.id} AND user_id = ${req.user.id}
      `;
    ...
    const [task] = await sql`
      UPDATE tasks SET
        ...
        time_estimate = COALESCE(${timeEstimate  ?? null}, time_estimate),
    ...
    if (status && status !== old.status) {
      const assignees = await sql`
        SELECT user_id FROM task_assignees WHERE task_id = ${req.params.id}
      `;
      for (const { user_id } of assignees) {
        if (user_id !== req.user.id) {
          await sql`
            INSERT INTO notifications (user_id, type, title, body)
            VALUES (
              ${user_id}, 'status', 'Task status updated',
              ${`"${old.title}" moved to ${status}`}
            )
          `;
        }
      }
    ...
    await sql`
      INSERT INTO activity_log (user_id, project_id, task_id, action, meta)
      VALUES (
        ${req.user.id}, ${task.project_id}, ${task.id},
        'task_updated', ${JSON.stringify({ changes: Object.keys(req.body) })}
      )
    `;
...
router.post('/:id/comments', [
  ...
  try {
    ...
    await sql`
      ...
      VALUES (${req.user.id}, ${task.project_id}, ${req.params.id}, 'comment_added')
    `;
...
router.post('/:id/time', [
  ...
  try {
    ...
    await sql`
      INSERT INTO time_logs (task_id, user_id, minutes, note)
      VALUES (${req.params.id}, ${req.user.id}, ${minutes}, ${note || null})
    `;
    await sql`
      UPDATE tasks SET time_logged = time_logged + ${minutes}
      WHERE id = ${req.params.id}
    `;
