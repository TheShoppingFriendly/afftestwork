  ...
  try {
    ...
    if (['admin', 'manager'].includes(role)) {
      projects = await sql`
        SELECT p.*,
          ...
          COALESCE(
            json_agg(DISTINCT jsonb_build_object(
              'id', pu.id, 'name', pu.name, 'avatar', pu.avatar, 'color', pu.color
        ...
        WHERE p.is_archived = ${isArchived}
    ...
    // Apply filters in JS to avoid nested sql`` parameter conflicts
  ...
  try {
    const [project] = await sql`
      SELECT p.*,
        ...
        COALESCE(
          json_agg(DISTINCT jsonb_build_object(
            'id', pu.id, 'name', pu.name, 'avatar', pu.avatar,
            'color', pu.color, 'role', pu.role, 'team', pu.team
          )) FILTER (WHERE pu.id IS NOT NULL),
        '[]') as team
      FROM projects p
      LEFT JOIN users u ON p.lead_id = u.id
...
router.post('/', authorize('admin', 'manager'), [
  ...
  try {
    const {
      name, type, client, status, priority,
      startDate, dueDate, budget, description,
      color, tags, leadId, teamIds
    } = req.body;

    const [project] = await sql`
      INSERT INTO projects (
        name, type, client, status, priority,
        start_date, due_date, budget, description,
        color, tags, lead_id, created_by
      ) VALUES (
        ${name},
        ${type || null},
        ${client || null},
    ...
    await sql`
      INSERT INTO activity_log (user_id, project_id, action, meta)
      VALUES (${req.user.id}, ${project.id}, 'project_created', ${JSON.stringify({ name })})
    `;
...
router.put('/:id', authorize('admin', 'manager', 'lead'), projectAccess, [
  ...
  try {
    const {
      name, type, client, status, priority,
      startDate, dueDate, budget, spent,
      description, color, tags, leadId, progress
    } = req.body;
    ...
    await sql`
      INSERT INTO activity_log (user_id, project_id, action, meta)
      VALUES (
        ${req.user.id}, ${project.id}, 'project_updated',
